...
class AnnotationWindow(QWidget):
    def __init__(self, text_content,file_name):
        ...
        self.setWindowIcon(QIcon("C:/Users/36057/Desktop/fakeVideoAnnotation_PyQt/resources/images/mannuate.png"))
        ...
        annotation_file = f"{os.path.splitext(self.file_name)[0]}_annotated.json"
    ...
    def save_and_close(self):
        ...
        output_file = os.path.join(f"{os.path.splitext(self.file_name)[0]}_annotated.json")

        # 分割存储的文本
        noun_text_list = self.noun_text.strip().split('\n')
        verb_text_list = self.verb_text.strip().split('\v')
        property_text_list = self.property_text.strip().split('\f')
...
class myMainWindow(Ui_MainWindow, QMainWindow):
    def __init__(self):
        ...
        self.frame_time_edit.setText("10") #设定默认帧时间为10秒
        ...
        self.Video_listWidget.itemClicked.connect(self.openSelectedFile)  #点击列表文件即可打开
        self.audio_listWidget.itemClicked.connect(self.openSelectedFile)
        self.Text_listWidget.itemClicked.connect(self.openSelectedFile)
        self.btn_Annotation.clicked.connect(self.show_Annotation_window)

    def show_data(self):
        # 调用计算数据的函数并显示数据
        video_count, average_duration, average_text = self.calculate_data()
    ...
    def calculate_data(self):
        # 计算已标注的视频数量
        video_count = self.Video_listWidget.count()

        # 计算平均视频时长
        total_duration = 0
        for item_index in range(self.Video_listWidget.count()):
            file_path = self.Video_listWidget.item(item_index).text()
        ...
        # 计算提取的文本平均数量
        total_text_count = 0
        for item_index in range(self.Text_listWidget.count()):
            file_path = self.Text_listWidget.item(item_index).text()
            with open(file_path, "r", encoding="utf-8") as file:
                total_text_count += len(file.read().split())
        average_text = total_text_count / self.Text_listWidget.count() if self.Text_listWidget.count() > 0 else 0

        return video_count, average_duration, average_text
    ...
    def upload_video(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select File", "", "Video Files (*.mp4 *.avi)")
        if files:
            for file in files:
