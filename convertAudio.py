...
class RequestApi(object):
    ...
    def all_api_request(self):
        ...
        file_path = self.upload_file_path+"Transcribed_text.txt"
        transcribed_text = ""
        print("JSON转字典后数据类型为："+str(type(t2)))
        for i in range(len(t2)):
          print(t2[i]['onebest'],end='')
          transcribed_text+=t2[i]['onebest']
        with open(file_path, "w", encoding="utf-8") as file:
            file.write(transcribed_text)
        print("转写结果已保存到文件：", file_path)
        return file_path

# 输入讯飞开放平台的appid，secret_key和待转写的文件路径
# if __name__ == '__main__':
#     api = RequestApi(appid="609c45ef", secret_key="ec26bac5dba71c1d419310ce2e01f649", upload_file_path=r"video\\fakeNews1_audio\\fakeNews1.wav")
#     api.all_api_request()
